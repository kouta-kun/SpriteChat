...
namespace SpriteChat.Server
{
    public class SpriteChatService
    {
        ...
        private void ExecuteCommand(ClientObject state, string command)
        {
            ...
            {
                state.name = command.Substring(2, command.Length - 3);
                SendStatus(state);
            }
        ...
        private void HandleMessage(ClientObject state, string command) => lastMessages.Add((state, command.Substring(1, command.Length - 2)));
